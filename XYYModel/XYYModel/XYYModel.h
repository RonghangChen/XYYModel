...
//2.属性类型为对象:首先判断值是否是同一种类对象，直接返回，否则执行默认转换策略进行转换（能进行默认转换的类NSString、NSMutableString、NSMutableArray、NSMutableDictionary、NSNumber、NSDecimalNumber、NSDate以及MyModel子类）,无法转换则返回nil
...
//2.1.value为对象,如果为nil则返回NSNull对象,其他情况调用convertToJsonValue方法进行转换,具体的默认转换策略参见convertToJsonValue方法定义
 ...
 * @param dicToModle dicToModle指示转换方向，YES为字典到模型，NO为模型到字典
 * @return 默认返回NO
